import { TEST_IDS } from '@/tests/helpers/objectid-helper';
import { setupDefaultMocks } from '@/tests/helpers/test-helpers';
import { describe, it, expect, vi, beforeEach } from 'vitest';
import { NextRequest } from 'next/server';
import { GET, POST } from '../route';
import { getServerSession } from 'next-auth';
import { prismaMock } from '@/tests/mocks/prisma';
import { themeColors } from '@/lib/theme';

vi.mock('next-auth');
vi.mock('@/lib/prisma');

describe('API /api/user/theme', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    setupDefaultMocks();
  });

  describe('GET', () => {
    it('devrait retourner 401 si non authentifié', async () => {
      vi.mocked(getServerSession).mockResolvedValue(null);

      const response = await GET();
      const data = await response.json();

      expect(response.status).toBe(401);
      expect(data.error).toBe('Non authentifié');
    });

    it('devrait retourner le thème de l\'utilisateur', async () => {
      vi.mocked(getServerSession).mockResolvedValue({
        user: { email: 'test@example.com' },
      } as any);

      prismaMock.user.findUnique.mockResolvedValue({
        themeColor: 'purple',
      } as any);

      const response = await GET();
      const data = await response.json();

      expect(response.status).toBe(200);
      expect(data.themeColor).toBe('purple');
    });

    it('devrait retourner "blue" par défaut si pas de thème', async () => {
      vi.mocked(getServerSession).mockResolvedValue({
        user: { email: 'test@example.com' },
      } as any);

      prismaMock.user.findUnique.mockResolvedValue(null);

      const response = await GET();
      const data = await response.json();

      expect(response.status).toBe(200);
      expect(data.themeColor).toBe('blue');
    });
  });

  describe('POST', () => {
    it('devrait retourner 401 si non authentifié', async () => {
      vi.mocked(getServerSession).mockResolvedValue(null);

      const request = new NextRequest('http://localhost:3000/api/user/theme', {
        method: 'POST',
        body: JSON.stringify({ themeColor: 'blue' }),
      });

      const response = await POST(request);
      const data = await response.json();

      expect(response.status).toBe(401);
      expect(data.error).toBe('Non authentifié');
    });

    it('devrait retourner 400 si couleur invalide', async () => {
      vi.mocked(getServerSession).mockResolvedValue({
        user: { email: 'test@example.com' },
      } as any);

      const request = new NextRequest('http://localhost:3000/api/user/theme', {
        method: 'POST',
        body: JSON.stringify({ themeColor: 'invalid-color' }),
      });

      const response = await POST(request);
      const data = await response.json();

      expect(response.status).toBe(400);
      expect(data.error).toBe('Couleur de thème invalide');
    });

    it('devrait mettre à jour le thème de l\'utilisateur', async () => {
      vi.mocked(getServerSession).mockResolvedValue({
        user: { email: 'test@example.com' },
      } as any);

      prismaMock.user.update.mockResolvedValue({
        id: TEST_IDS.user1,
        themeColor: 'emerald',
      } as any);

      const request = new NextRequest('http://localhost:3000/api/user/theme', {
        method: 'POST',
        body: JSON.stringify({ themeColor: 'emerald' }),
      });

      const response = await POST(request);
      const data = await response.json();

      expect(response.status).toBe(200);
      expect(data.themeColor).toBe('emerald');
      expect(prismaMock.user.update).toHaveBeenCalledWith({
        where: { email: 'test@example.com' },
        data: { themeColor: 'emerald' },
      });
    });
  });
});
